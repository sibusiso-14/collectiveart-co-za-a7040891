...
export const Route = createFileRoute("/admin/ambassadors")({
  component: AdminAmbassadorApplications,
});
...
function AdminAmbassadorApplications() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (fetchError) {
        setError("Could not load ambassador applications. You may not have admin access, or you're not signed in.");
      } else {
        setRows(data as Row[]);
      }
      setLoading(false);
  ...
  return (
    <div className="mx-auto max-w-4xl px-5 py-14 md:px-10 md:py-20">
      <h1 className="font-serif text-4xl md:text-5xl">Ambassador Applications</h1>
